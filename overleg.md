...
### 2. Slimmer scrapen (efficiëntie) — hash-caching GEBOUWD 2026-08-14, parallelle requests UITGEWERKT 2026-08-16 (nog te bouwen)
...
**Parallelle requests — aanpak vastgelegd 2026-08-16, nog te bouwen** (Michiel: "eerst alleen vastleggen, later oppakken"). Twee onafhankelijke niveaus, beide in scope (A+B):

- **Niveau A — tussen scrapers**: `run_weekly_refresh.py`'s hoofdlus draait nu 56 `scrape_*.py`-subprocessen na elkaar. Wordt een `ThreadPoolExecutor` rond de bestaande `run_one()`-subprocess-call, met **aparte concurrency-limieten per scraper-type** (Michiel's keuze, i.p.v. één globale limiet): plain-HTTP-scrapers wegen licht, Playwright-scrapers (11 stuks, elk een eigen Chromium-proces) wegen zwaar qua geheugen. Scraper-type wordt herkend door het bestand te grep'en op `"playwright"` (geen aparte config per script nodig). Voorstel-startwaarden (niet hard vastgezet, bijstellen na een eerste echte run): **8 gelijktijdig voor plain-HTTP**, **3 gelijktijdig voor Playwright**.
- **Niveau B — binnen één scraper**: de paginaloop van een scraper zelf parallelliseren i.p.v. sequentieel. Raakt **niet** alle 56 bestanden — alleen de 7 met een echte multi-request paginaloop: `scrape_drenthe.py` (34+ pag.), `scrape_friesland.py` (~69 pag.), `scrape_visitgroningen.py`, `scrape_forum.py`, `scrape_kielzog.py`, `scrape_posthuistheater.py`, `scrape_paard.py`. Concertgebouw/GelreDome hebben ook paginering maar via Playwright met al één hergebruikte browser — bewust **buiten scope** gehouden voor deze ronde (ander soort complexiteit: losse browser-tabs/contexts i.p.v. gewoon meer HTTP-requests tegelijk), evt. apart oppakken later.
  - Bouwplan: nieuwe gedeelde helper (bv. `parallel_fetch.py`, zelfde soort klein infra-bestand als `ssl_fix.py`/`page_cache.py`/`ticketmaster.py` — geen scraping-logica erin, dus geen conflict met de "één bestand per bron"-afspraak, zie punt 8/decisions.md) met een `fetch_pages(urls, max_workers=5)`-achtige functie op basis van `ThreadPoolExecutor` + het bestaande `urllib` (geen nieuwe dependency nodig — alle 7 kandidaten gebruiken al `urllib`, niet `requests`). Concurrency **per scraper bewust laag** (voorstel: 5 gelijktijdige requests naar dezelfde bron) — dit zijn nu eenmaal-vriendelijke sites bij één sequentiële request per keer; te veel gelijktijdige connecties naar dezelfde domain kan alsnog rate-limiting/bot-detectie triggeren die er nu niet is.
  - Change-detection blijft ongewijzigd: `unchanged()` zit ná het verzamelen van alle pagina's (zie ARCHITECTURE.md §Change-detection) — of het ophalen sequentieel of parallel gebeurt maakt voor die check niets uit.
- **Randvoorwaarde voor Niveau A, moet er sowieso bij**: alle scrapers schrijven nu naar dezelfde SQLite-file via `insert_event()` (`events_db.py:83`) zonder WAL-mode of busy-timeout. Bij écht gelijktijdige processen kan dat een "database is locked"-fout geven. Fix: `PRAGMA journal_mode=WAL` + een `busy_timeout` bij het openen van de connectie in `events_db.py` — klein, bekend patroon, moet vóór Niveau A gebouwd worden (of gelijktijdig).
- **Nog open bij het bouwen**: exacte pool-groottes (voorstel-getallen hierboven, aanpassen na een eerste echte parallelle run), en of de output van `run_weekly_refresh.py` per-script gebufferd/gelabeld getoond wordt (voltooiingsvolgorde i.p.v. opgave-volgorde bij parallel draaien).
