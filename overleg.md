...
### 13. Exposities uitbreiden: meer musea/galerieën als bron — NIEUW 2026-08-17
...
- **Nog te bepalen, voor er gebouwd wordt:**
  - Eerst Kunstpunt Groningen checken als mogelijke aggregator (potentieel grootste winst per scraper-bouw-inspanning), vóór het overwegen van losse galerie-scrapers?
  - Scope: alleen Groningen stad, of ook Drenthe/Friesland-musea en -galerieën meenemen (sluit aan bij overleg.md punt 6, landelijke/regionale uitbreiding in het algemeen)?
  - Ondergrens: een kleine galerie met misschien 1-2 exposities per jaar geeft weinig terug voor de bouw-/onderhoudsinspanning van een eigen scraper — moet er een minimale drempel komen (bv. alleen bronnen met een regelmatige/actieve agenda)?
  - GRID Grafisch Museum/Universiteitsmuseum/Synagoge Groningen: aparte kleine musea, elk een eigen "is dit scrapebaar zonder AI"-check nodig (nog niet gedaan).
- Nog geen keuze gemaakt of gebouwd — bewust eerst vastgelegd zodat de scope bepaald kan worden voor er individuele scrapers gebouwd worden.
