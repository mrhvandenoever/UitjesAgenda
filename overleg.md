...
- ⬜ **Bewust overgeslagen, nu heroverwogen en definitief niet nodig (2026-08-18)**: lazy-loading-architectuur (alleen eerstvolgende ~60 dagen server-side, rest per maand als JSON). Michiel vroeg zich af of het niet-bouwen ervan een grote afwijking van de norm is ("5,5MB vind ik wel meevallen"). Gemeten op de live site (Performance/Resource Timing API): `decodedBodySize` is inderdaad ~5,1MB, maar `transferSize` (echt over de lijn, Cloudflare's brotli/gzip-compressie) is maar **~444KB** in **1 request**, DOMContentLoaded ~600ms. Dat is lichter en sneller dan een gemiddelde website (HTTP Archive-mediaan ligt rond 2,2-2,7MB transferred / 70-100+ requests) — de eerder gerapporteerde "5,5MB" was de ongecomprimeerde HTML-grootte, geen representatieve performance-metric. Lazy-loading blijft daarom hier geparkeerd, niet als "nog te doen" maar als optie voor een moment dat de dataset fors groeit (bv. 5-10x meer bronnen) en dit opnieuw gemeten wordt.
