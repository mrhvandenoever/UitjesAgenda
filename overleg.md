...
### 19. Derde Claude Design-ronde: grotere voorstellen nog te prioriteren (2026-08-18)
- Aanleiding: na cluster 5 vroeg Michiel Claude Design nogmaals om feedback op de branch-preview (ditmaal een statische HTML/CSS/JS-analyse, geen live klik-test). Vond een echte blokkerende bug (Wanneer-filter deed niets, `#uitjes-datum`→`#popover-when` gemist bij de cluster-5-rename) + 2 kleinere regressies + een paar kleine nieuwe bugs — allemaal bevestigd en gefixt, zie decisions.md 2026-08-18. Dit punt is voor de rest: grotere/subjectieve voorstellen uit hetzelfde rapport.
- **Kaart-layout-herstructurering** (door Claude Design zelf "het grootste visuele probleem" genoemd): op brede schermen duwt het `70px 1fr auto`-grid de badges ~2000px van de titel af — voorstel `main{max-width:1000px;margin:0 auto}` (leeslijst i.p.v. tabel) + badges dichter bij de titel/venue-regel + de datum niet per rij herhalen maar een dag-groepskop (zoals de bestaande maand-koppen, één niveau dieper).
- **URL-state uitbreiden**: mist nu `when`/`sport`/`club`/`gender`/`sort` — "Dit weekend in Drenthe" is niet deelbaar. Ook: `d=25` gaat wel mee maar het middelpunt (adres/coördinaten) niet, dus dezelfde link betekent bij de ontvanger iets anders (afstand rond een ander punt).
- **localStorage-persistentie**: niets wordt onthouden tussen bezoeken — met name het adres is iets dat een gebruiker maar één keer wil intypen.
- **Zoek-normalisatie**: `data-search.includes(q)` is te letterlijk — geen diakrieten-folding ("zummerbuhne" vindt "Zummerbühne" niet) en geen meerdere-woorden-ondersteuning (moet exacte substring zijn i.p.v. AND van losse woorden).
- **Lege-staat-actieknoppen**: bericht is goed, maar geen directe uitweg (bv. een "Wis filters"/"Alle afstanden"-knop erin) — gebruiker moet nu zelf terug naar de toolbar.
- **Mobiele toolbar-herindeling**: zoekveld staat nu in dezelfde horizontaal-scrollende rij als de filterknoppen (70vw breed) — bij openen zie je het zoekveld + 1 knop, de rest buiten beeld. Voorstel: zoekveld op een eigen volle regel, alleen de knoppen zelf scrollen.
- **Chips in popovers nog 36px** (touch-target-doel was 44px, maar dat is alleen op de toolbar-knoppen zelf toegepast — het echte tikken gebeurt op de chips erin).
- **Typografie, nog steeds grotendeels ongewijzigd sinds de eerste review**: titel (0.95rem/≈13px) is nog kleiner dan de body-basis (14px) — moet juist het sterkste element zijn, voorstel 15-16px. `--muted` (#757575) op de achtergrondkleur (#f9f9f9, niet zuiver wit) is 4,4:1, net onder de 4,5 die kleine tekst nodig heeft — `#6b6b6b` lost dat op.
- **Kleinere, mogelijk-geen-bug punten**: lege venue-regel bij events zonder venue ÉN zonder city (val terug op iets, of laat de regel weg) — datakwaliteit, geen codefix; provincielijst mist Zeeland/Limburg/Flevoland (bewust, geen bronnen daar — geen actie nodig, hooguit een keer expliciet vermelden); een "alleen bronnen binnen mijn afstand"-schakelaar bovenaan de Bron-popover zou de muur van 60 chips daar verkleinen.
